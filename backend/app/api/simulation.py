...
# Adding this prefix prevents Agent from calling tools and makes it reply with text directly
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in the backend/scripts/ directory, no longer copied to simulation directory
...
def prepare_simulation():
    ...
    This is a time-consuming operation, the API returns a task_id immediately,
    use GET /api/simulation/prepare/status to check progress
    
    Features:
    - Auto-detects completed preparation work, avoids redundant generation
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Preparation work already exists",
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Checking simulation {simulation_id} if already prepared...")
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation work already exists, no need to regenerate",
...
def get_prepare_status():
    ...
    2. Check if preparation work is already complete via simulation_id
    ...
    try:
        ...
        # if simulation_id provided, check firstwhether already prepared
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Preparation work already exists",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Not yet prepared, please call /api/simulation/prepare to start",
                        "already_prepared": False
                    }
                })
            return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    if multiple, return the newest (sorted by created_at)
...
def get_simulation_history():
    ...
    For homepage historical project display, returns simulation list with rich info including project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, or recommended rounds if not available
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time get Agent Profiles for a simulation (for viewing progress during generation）
    ...
    - Returns extra metadata (e.g., file modification time, whether currently generating, etc.)
    ...
    try:
        ...
        logger.error(f"real-timeFailed to get profiles: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g., file modification time, whether currently generating, etc.)
    - Can return partial info even if config generation is not complete
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: LLM config reasoning explanation
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",          // required，Simulation ID
            "platform": "parallel",                // options: twitter / reddit / parallel (default)
            "max_rounds": 100,                     // options: maximum simulation rounds for truncating longSimulation 
            "enable_graph_memory_update": false,   // options: whether to dynamically update Agent activities to Zep graph memory
    ...
    About the force parameter:
        ...
        - Will not clean up config files (simulation_config.json) or profile files
        - Suitable for scenarios requiring simulation re-runs
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // whether force restart
            }
        }
    """
    try:
        ...
        max_rounds = data.get('max_rounds', Config.OASIS_DEFAULT_MAX_ROUNDS)  # default uses config value
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"invalid platformtypes: {platform}，options: twitter/reddit/parallel"
            }), 400

        # Checking simulationwhether prepared
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    # Checking simulationis process actually running
...
def get_run_status_detail(simulation_id: str):
    ...
    Query parameters:
        platform: Platform filter (twitter/reddit, optional)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "twitter_actions": [...],  # Twitter all actions for platform
                "reddit_actions": [...]    # Reddit all actions for platform
            }
        }
    """
    try:
...
def get_simulation_comments(simulation_id: str):
    ...
    Query parameters:
        post_id: Post ID filter (optional)
        limit: Return count
        offset: Offset
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (enters command waiting mode after simulation cycle)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // when not specified: interview both platforms simultaneously in dual-platform simulation
            "timeout": 60                      // optional，timeout in seconds），default60
        }
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation has completed and entered command waiting mode。"
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required，interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your opinion on B？"  // if platform not specified, use default
                }
            ],
            "platform": "reddit",              // optional，default platform (overridden by each item platform)
                                               // when not specified: interview both platforms for each Agent in dual-platform simulation
            "timeout": 120                     // optional，timeout in seconds），default120
        }
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"interview list item{i+1}item missing agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"interview list item{i+1}item missing prompt"
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"interview list item{i+1}item's platform must be 'twitter' or 'reddit'"
                }), 400
        ...
        # optimize each interview item prompt, add prefix to prevent Agent from calling tools
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",                   // optional, specify platform (twitter/reddit)
                                                    // when not specified: interview both platforms for each Agent in dual-platform simulation
            "timeout": 180                          // optional，timeout in seconds），default180
        }
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "environment running, can receive interview commands"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    Note: this is different from /stop, which forcefully terminates the process,
    while this endpoint lets the simulation gracefully close the environment and exit。
    
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",  // required，Simulation ID
            "timeout": 30                  // optional，timeout in seconds），default30
        }
