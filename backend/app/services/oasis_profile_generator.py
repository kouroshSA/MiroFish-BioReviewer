...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface, so we search edges and
        nodes separately then merge the results. Uses parallel requests for efficiency.
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if after the last quote there is no comma or closing bracket, the string may be truncated
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""IMPORTANT: All output must be in English only. Do not use any other language.

Generate a detailed social media user persona for the entity, restoring existing real-world situations as much as possible.
...
Generate a detailed social media account profile for the institution/group entity, restoring existing real-world situations as much as possible.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   - Institution basic information (official name, nature, founding background, main functions)
   ...
   - Institution memory (important part of the persona, introduce this institution's connection to events, and its existing actions and reactions in the events)
...
- Use English for all fields (gender field must be English "other")
...
- Institutional account communication must match its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        def save_profiles_realtime():
            ...
            with lock:
                ...
                try:
                    # Write to temp file then atomically rename to avoid
                    # read-write races with the frontend polling the file.
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # description: Brief bio, used for external display
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Gender mapping (including Chinese fallback)
        gender_map = {
            "male": "male",
            "female": "female",
            "organization": "other",
