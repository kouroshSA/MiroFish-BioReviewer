...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation completion (content only, does not mean entire section is done)"""
        self.log(
...
class ReportOutline:
    ...
    def to_markdown(self) -> str:
        """Convert to Markdown format (used by UI display)"""
        md = f"# Report Agent's Take: {self.title}\n\n"
        md += f"> {self.summary}\n\n"
...
This is not LLM simulation, but calls the real interview interface to get simulation Agents' original responses.
...
PLAN_SYSTEM_PROMPT = """\
IMPORTANT: All output must be in English only. Do not use any other language.
...
3. [Language Consistency - All Content Must Be in English]
   - Tool-returned content may contain text in various languages
   - The report must be written entirely in English
   - When quoting tool-returned content in other languages, you must translate it into fluent English before including it in the report
...
[Tool Usage Recommendations - Please mix different tools, do not use only one type]
...
REACT_UNUSED_TOOLS_HINT = "\nTip: You have not yet used: {unused_list}. Consider trying different tools for multi-perspective information."
...
"""
        ...
        try:
            ...
            if Config.SIMULATION_MODE == 'biological':
                return ReportOutline(
                    ...
                    summary="Key findings from the biological entity simulation",
                    sections=[
                        ReportSection(title="Key Findings: Dynamic Changes in Biological Networks"),
                        ReportSection(title="Biological Entity Behavior Analysis"),
                        ReportSection(title="Emerging Trends and Risk Assessment")
                    ]
                )
            else:
                return ReportOutline(
                    ...
                    sections=[
                        ...
                        ReportSection(title="Trend Outlook and Risk Assessment")
                    ]
                )
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            # Use this content directly as final answer, stop iterating
            logger.info(f"Section {section.title}: no 'Final Answer:' prefix detected, using LLM output directly as final content (tool calls: {tool_calls_count})")
    ...
    def _generate_discussion_section(
        ...
        """
        Generate an academic Discussion section by comparing the original
        uploaded document with the generated simulation report.
        Uses a stronger LLM model for higher quality scientific writing.
        """
        from .simulation_manager import SimulationManager
        from ..models.project import ProjectManager
        from ..utils.file_parser import FileParser

        logger.info("Generating Discussion section using model: %s", Config.DISCUSSION_LLM_MODEL_NAME)

        # 1. Resolve original document path
        manager = SimulationManager()
        state = manager.get_simulation(self.simulation_id)
        if not state:
            logger.warning("Cannot find simulation %s, skipping Discussion", self.simulation_id)
            return ""

        # Get actual file paths from the project files directory
        file_paths = ProjectManager.get_project_files(state.project_id)
        if not file_paths:
            logger.warning("No project files found for %s, skipping Discussion", state.project_id)
            return ""

        # 2. Extract text from the original document
        original_text = ""
        for file_path in file_paths:
            try:
                original_text += FileParser.extract_text(file_path) + "\n\n"
                logger.info("Extracted text from %s (%d chars)", file_path, len(original_text))
            except Exception as e:
                logger.warning("Failed to extract text from %s: %s", file_path, e)

        if not original_text.strip():
            logger.warning("Could not extract text from uploaded documents, skipping Discussion")
            return ""

        # 3. Gather report text
        report_text = "\n\n---\n\n".join(generated_sections)

        # 4. Truncate if needed (generous limits for strong model)
        max_chars = 200000
        ...
        # 5. Create a separate LLM client with the stronger model
        discussion_llm = LLMClient(model=Config.DISCUSSION_LLM_MODEL_NAME)

        # 6. Build the prompt
        system_prompt = (
            "You are an academic scientific writer producing a Discussion section "
            "for a peer-reviewed journal article. Write in third person, past tense, "
            "formal academic register. Do NOT use first person. Do NOT reference any "
            "'agent', 'simulation agent', 'tool', 'system prompt', or 'AI'. "
            "Treat the simulation findings as computational modeling results. "
            "Do NOT use conversational tone. Use Markdown formatting."
        )

        user_prompt = f"""Below are two texts:

## ORIGINAL PAPER
{original_text}

## COMPUTATIONAL MODELING REPORT
{report_text}
...
1. Synthesizes the key findings from the computational modeling in the context of the original paper's data and conclusions.
2. Identifies areas of agreement between the modeling results and the experimental/clinical evidence in the original paper.
3. Highlights novel insights or predictions that emerged from the computational modeling that were not explicitly stated in the original paper.
4. Discusses areas of divergence or unexpected findings, and provides possible explanations.
5. Addresses limitations of the computational modeling approach.
6. Suggests specific directions for future experimental and computational research.
7. **Concludes with a "big picture" paragraph** that places the overall significance of these findings within the broader context of the field — what do these results mean for the future of this area of research?
...
[big-picture concluding paragraph]
"""

        if progress_callback:
            progress_callback("generating", 90, "Generating Discussion section (using advanced model)...")

        if self.report_logger:
            self.report_logger.log_section_start("Discussion", section_index)

        try:
            response = discussion_llm.chat(
                messages=[
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": user_prompt},
                ],
            ...
            logger.error("Discussion generation failed: %s", e)
            return ""

        if not response or not response.strip():
            logger.warning("Discussion section LLM returned empty response")
            return ""

        # Log token usage from the discussion model
        disc_usage = discussion_llm.get_usage_summary()
        logger.info(
            "Discussion token usage (model=%s): prompt=%d, completion=%d, total=%d",
            Config.DISCUSSION_LLM_MODEL_NAME,
            disc_usage["prompt_tokens"], disc_usage["completion_tokens"], disc_usage["total_tokens"],
        )
        if self.report_logger:
            self.report_logger.log(
                action="discussion_token_usage",
                stage="generating",
                details={"model": Config.DISCUSSION_LLM_MODEL_NAME, **disc_usage}
            )

        # Ensure it starts with ## Discussion
        content = response.strip()
        if not content.startswith("## Discussion"):
            content = "## Discussion\n\n" + content
    ...
    def _generate_big_picture_section(
        self,
        generated_sections: List[str],
        ...
        """
        Generate a Big Picture wrap-up paragraph suitable for a high-tier
        scientific paper's discussion section. Uses the stronger Discussion model.
        """
        logger.info("Generating Big Picture section using model: %s", Config.DISCUSSION_LLM_MODEL_NAME)
        ...
        system_prompt = (
            "You are an academic scientific writer for a high-tier peer-reviewed journal. "
            "Write in third person, past tense, formal academic register. "
            "Do NOT use first person. Do NOT reference any 'agent', 'simulation agent', "
            "'tool', 'system prompt', or 'AI'. "
            "IMPORTANT: All output must be in English only. Do not use any other language."
        )

        user_prompt = f"""Below is a computational modeling report and its discussion section:
...
Based on the report and discussion above, write a single concise "Big Picture" wrap-up paragraph that:

1. Synthesizes the overarching insight that has emerged from the collective findings
2. Places this insight within the broader context of the field
3. Articulates the broad implications for future research and clinical practice
4. Is written in a tone and style suitable as the concluding paragraph of a Discussion section in a high-tier scientific journal (e.g., Nature, Cell, Science)

Output ONLY the paragraph — no heading, no preamble, no bullet points. Just one powerful concluding paragraph.
"""

        if progress_callback:
            progress_callback("generating", 95, "Generating Big Picture wrap-up...")
        ...
        try:
            ...
            logger.error("Big Picture generation failed: %s", e)
            return ""

        if not response or not response.strip():
            logger.warning("Big Picture section LLM returned empty response")
            return ""

        # Log token usage
        bp_usage = discussion_llm.get_usage_summary()
        logger.info(
            "Big Picture token usage (model=%s): prompt=%d, completion=%d, total=%d",
            Config.DISCUSSION_LLM_MODEL_NAME,
            bp_usage["prompt_tokens"], bp_usage["completion_tokens"], bp_usage["total_tokens"],
        )
        if self.report_logger:
            self.report_logger.log(
                action="big_picture_token_usage",
                stage="generating",
                details={"model": Config.DISCUSSION_LLM_MODEL_NAME, **bp_usage}
            )
    ...
    def generate_report(
        ...
        try:
            ...
            for i, section in enumerate(outline.sections):
                section_num = i + 1
                base_progress = 20 + int((i / total_sections) * 68)
                ...
                ReportManager.update_progress(
                    ...
                    base_progress + int(68 / total_sections),
            ...
            logger.info(
                "Token usage for report %s: prompt=%d, completion=%d, total=%d",
                report_id, token_usage["prompt_tokens"],
                token_usage["completion_tokens"], token_usage["total_tokens"],
            )
            if self.report_logger:
                self.report_logger.log(
                    action="token_usage_summary",
                    stage="completed",
                    details=token_usage
                )
...
class ReportManager:
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble complete report from saved section files and perform heading cleanup
        """
        folder = cls._get_report_folder(report_id)
        
        # Build report header — saved version uses "LLM Refined Report" prefix
        md_content = f"# LLM Refined Report: {outline.title}\n\n"
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        3. Clean up excessive blank lines and separator lines
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it's a duplicate heading (same title within 5 consecutive lines)
                is_duplicate = False
