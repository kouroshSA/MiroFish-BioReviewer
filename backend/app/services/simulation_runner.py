...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # Create main log file, avoid process blocking from stdout/stderr pipe buffer overflow
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess env vars, ensure UTF-8 encoding on Windows
            ...
            # Set working directory to simulation dir (database files will be generated here)
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensuring all related processes can be terminated on server shutdown
            )
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            # If graph memory update enabled, send activities to Zep
                            if graph_updater:
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forced simulation restart)
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        # process may no longer exist, trying direct termination
                        try:
                            process.terminate()
                    ...
                    # simultaneously update state.json，set status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
                ...
                logger.error(f"clean up processesfailed: {simulation_id}, error={e}")
    ...
    def register_cleanup(cls):
        ...
        called at Flask app startup to ensure all simulation processes are cleaned up on server shutdown
        """
        ...
        # Flask debug in debug mode, only register cleanup in reloader subprocess (the process actually running the app）
        # WERKZEUG_RUN_MAIN=true indicates reloader subprocess
        # if not debug mode, this env var does not exist, registration is also needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # in debug mode, only register in reloader subprocess；always register in non-debug mode
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # only print log when processes need cleanup
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True True means environment alive, False means environment closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: specify platform (optional）
                ...
                - None: interview both platforms simultaneously in dual-platform simulation, return consolidated results
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment not running or closed, cannot executeInterview: {simulation_id}")
    ...
    def interview_all_agents(
        ...
        interview all Agents in the simulation with the same questionAgent
        ...
        if not agent_configs:
            raise ValueError(f"Simulation config does not containAgent: {simulation_id}")
    ...
    def get_interview_history(
        ...
        # if queried multiple platforms, limit total
