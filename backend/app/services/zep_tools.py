...
Wraps graph search, node reading, edge queries and other tools for use by Report Agent.
...
2. PanoramaSearch (Broad Search) - Gets full picture, including expired content
...
class ZepToolsService:
    ...
    2. panorama_search - Broad search (gets full picture, including expired content)
    3. quick_search - Simple search (quick retrieval)
    4. interview_agents - In-depth interview (interviews simulated agents, gets multi-perspective viewpoints)
    ...
    def search_graph(
        ...
        """
        # Zep Cloud limits queries to 400 characters
        if len(query) > 400:
            query = query[:400]
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all graph edges, then filters for edges related to the specified node.
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """IMPORTANT: All output must be in English only. Do not use any other language.
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        3. Categorizes currently active and historical information

        This tool is suitable for scenarios that require understanding the full picture of events and tracking evolution.
    ...
    def interview_agents(
        ...
        try:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may be shut down, please ensure OASIS environment is running."
            return result
        except Exception as e:
...
You are a professional interview planning expert. Your task is to select the most suitable interviewees from a list of simulated agents based on interview requirements.
...
You are a professional journalist/interviewer. Based on interview requirements, generate 3-5 in-depth interview questions.
...
Please generate 3-5 interview questions."""
        ...
        try:
            ...
            return [
                f"What is your viewpoint on {interview_requirement}?",
