...
Uses LLM to automatically generate detailed simulation parameters based on simulation
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost nobody during early morning
...
class SimulationConfigGenerator:
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        prompt = f"""IMPORTANT: All output must be in English only. Do not use any other language.
...
### Basic Principles (for reference only, should be adjusted based on specific events and participant groups):
- The user group is Chinese, must follow Beijing time daily routine
- Midnight 0-5: almost no activity (activity coefficient 0.05)
- Morning 6-8: gradually active (activity coefficient 0.4)
- Work hours 9-18: moderately active (activity coefficient 0.7)
- Evening 19-22: peak period (activity coefficient 1.5)
- After 23: activity declining (activity coefficient 0.5)
- General pattern: low activity at dawn, gradual increase in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only. You need to adjust based on event nature and participant group characteristics
  - Example: Student groups may peak at 21-23; media active all day; government agencies only during work hours
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. IMPORTANT: All output must be in English only. Return pure JSON format. Time configuration must follow Chinese daily routine patterns."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration results and validate agents_per_hour values do not exceed total agent count"""
        # Get raw values
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost nobody during early morning
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. IMPORTANT: All output must be in English only. Return pure JSON format. Note that poster_type must exactly match available entity types."
...
- **Schedule follows Chinese daily routine**: Almost no activity from 0-5, most active during 19-22
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<active hour list, considering Chinese daily routine>],
        ...
        system_prompt = "You are a social media behavior analysis expert. IMPORTANT: All output must be in English only. Return pure JSON. Configuration must follow Chinese daily routine patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent configuration based on rules (Chinese daily routine)"""
