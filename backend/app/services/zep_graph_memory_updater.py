...
class AgentActivity:
    ...
    def _describe_follow(self) -> str:
        """Follow user - includes the followed user name"""
    ...
    def _describe_mute(self) -> str:
        """Mute user - includes the muted user name"""
        target_user_name = self.action_args.get("target_user_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors the simulation actions log file and updates new agent activities to the Zep graph in real time.
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-grouped activity buffers (each platform accumulates to BATCH_SIZE before batch sending)
        ...
        # Statistics
        self._total_activities = 0  # Actual activities added to the queue
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain full context information (e.g., original post content, username, etc.).
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Send interval, avoid sending requests too quickly
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Failed to batch send to Zep (attempt {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Failed to batch send to Zep after{self.MAX_RETRIES} retries: {e}")
